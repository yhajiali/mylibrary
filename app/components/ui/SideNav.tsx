...
export const SideNav = ({ openSideNav }: Props) => {
  ...
  return (
    <aside
      ...
      } transition-transform duration-300 lg:translate-x-0 flex flex-col justify-between fixed w-60 h-screen border-r bg-white`}
    >
      <nav className="flex flex-col gap-4 h-full py-4">
        {sideNavLinks.map((navSection) => (
          <ul className="flex flex-col" key={navSection.title}>
